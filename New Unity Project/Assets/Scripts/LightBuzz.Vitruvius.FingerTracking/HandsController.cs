...
namespace LightBuzz.Vitruvius.FingerTracking
{
    ...
    public class HandsController
    {
        ...
        // Flag for data aquisition
        public bool recordOn { get; set; }

        // Information for the data aquisition
        ...
        public HandsController()
        {
            ...
            numberOfSamples = 50;
            grahamPoints = new int[numberOfSamples, 3];
            contourPoints = new int[numberOfSamples, 3];
        ...
        private Hand GetHand(ulong trackingID, HandState state, List<DepthPointEx> contour, double angle, float wristX, float wristY)
        {
            ...
            TimeSpan elapsedTime =  DateTime.Now - lastSampleTime ;
            if (recordOn && (sampleCounter < numberOfSamples) && (elapsedTime.TotalMilliseconds>200))
            {
                sampleCounter++;



                foreach (DepthPointEx point in convexHull)
                {

                    data.Add(point.toString() + "\n");
                }
                string fPath = @"C:\Users\Lu\Documents\Visual Studio 2015\Projects\Kinect-Finger-Tracking-master\Data\" + gesture + @"\" + "graham_" + subject + "_" + printID;
                System.IO.File.WriteAllLines(fPath, data.ToArray());


                data = new List<String>();
                foreach (DepthPointEx point in contour)
                {
                    data.Add(point.toString() + "\n");
                }

                fPath = @"C:\Users\Lu\Documents\Visual Studio 2015\Projects\Kinect-Finger-Tracking-master\Data\" + gesture + @"\" + "contour_" + subject + "_" + printID;
                System.IO.File.WriteAllLines(fPath, data.ToArray());
                printID += 1;

                lastSampleTime = DateTime.Now;
            }
            else if(sampleCounter == numberOfSamples)
            {
            ...
            //Console.Write(convexHull.Count + "\n");
